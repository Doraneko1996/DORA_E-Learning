...
<template>
    ...
    <div class="h-full flex flex-col border-r border-surface-200 dark:border-surface-800">
                ...
                <template #item="{ item, props }">
                    <router-link v-if="item.route" :to="item.route" v-slot="{ navigate, href }" custom>
                        <a v-ripple :href="href" v-bind="props.action" @click="navigate" :class="{
                            ...
                            <span v-if="item.shortcut" class="ml-auto text-xs bg-emphasis text-muted-color p-1 rounded">
                                {{ item.shortcut }}
                            </span>
                        </a>
                    </router-link>
        ...
        <div class="flex-none mt-auto p-3">
            <Button @click="toggleUserMenu" aria-haspopup="true" aria-controls="user_menu" v-ripple severity="secondary"
