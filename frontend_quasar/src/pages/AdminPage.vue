...
const openAdminForm = (user: User, editMode: boolean) => {
  $q.dialog({
    component: UserForm,
    componentProps: {
      persistent: true,
...
const openCreateModal = () => {
  ...
  openAdminForm(newAdmin, true)
}

// Mở modal chỉnh sửa
const handleEdit = (admin: User) => {
  openAdminForm({ ...admin }, false) // Mở ở chế độ xem chi tiết
}
...
const handleStatusChange = async (item: User, newStatus: boolean) => {
  ...
  try {
    loading.value = true
    const response = await adminService.updateStatus([item.id], newStatus)
...
const confirmDeleteAction = async (ids: number[]) => {
  ...
  try {
    loading.value = true
    const response = await adminService.delete(ids)
    notif({
      spinner: false,
      icon: 'done',
      color: 'positive',
...
const confirmResetPassword = async (ids: number[]) => {
  ...
  try {
    loading.value = true;
    const response = await adminService.resetPassword(ids);
    notif({
      spinner: false,
      icon: 'done',
      color: 'positive',
...
<template>
  <div class="q-pa-md">
    <DataTable ref="tableRef" endpoint="/admins/list" :headers="[
